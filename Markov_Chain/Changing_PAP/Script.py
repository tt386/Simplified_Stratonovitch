...
#Pesticide_Application_Period = 1
Pesticide_SS = 1#0.86
Pesticide_RR = 0.003
dR = 48.6
...
##############################################################################
#SaveFileCreation
SaveFileName = ("Saved_Plots/PAPjumps_%dYears_%d_OSRProportion_%0.3f_gSS_%0.3f_Characteristic_%0.3f_RefugeAttract_%0.3f_GradientRange_%d_BreedingTime_%d"%
                (jump,Years,OSR_Proportion,Pesticide_SS,Characteristic,Refuge_Attract,GradientRange,BreedingTime))
if FLATPESTICIDE:
...
for Pesticide_Application_Period in PAPs:
    InitialSlopeList = []
    
    print("PAP",Pesticide_Application_Period)
    ...
    SystemSizes = np.arange(Denominator,500,Denominator)
    ...
    for i in range(1,len(InitialSlopeList)-1):
        if (InitialSlopeList[i+1] > InitialSlopeList[i]) and (InitialSlopeList[i-1] > InitialSlopeList[i]):
            ...
            MinPAPs.append(Pesticide_Application_Period)
            break
    ...
    if len(minpointindex) > 0:
        ...
        SmoothedPAPs.append(Pesticide_Application_Period)
    ...
    plt.title("PAP: %d"%(Pesticide_Application_Period))
    plt.savefig(SaveFileName + "/InitialSlopePlot_PAP_" + str(Pesticide_Application_Period).zfill(5) + ".png") 
    plt.close()
