...
OSR_Proportions = np.arange(0.05,1.0,0.05)
...
Pesticide_Application_Period = Days#10
Pesticide_SS = 1#0.86
Pesticide_RR = 0#0.003
dR = 48.6
...
##############################################################################
#SaveFileCreation
SaveFileName = ("Saved_Plots/Years_%d_PAP_%d_gSS_%0.3f_gRR_%0.3f_Characteristic_%0.3f_RefugeAttract_%0.3f_GradientRange_%d_BreedingTime_%d"%
                (Years,Pesticide_Application_Period,Pesticide_SS,Pesticide_RR,Characteristic,Refuge_Attract,GradientRange,BreedingTime))
if FLATPESTICIDE:
...
for OSR_Proportion in OSR_Proportions:
    ...
    #Choose the smallest step size of the system sizes, based on the 
    #OSR proportion: find the denominator of the simplest fraction and
    #usr that as the step size and smallest size of the system
    ...
    SystemSizes = np.arange(Denominator,600,Denominator)
    ...
    for SystemSize in SystemSizes:
        ...
        SSPop = EigenVect * (1.-R_Allele_Ratio)
        SRPop = EigenVect * 0#2*R_Allele_Ratio*(1.-R_Allele_Ratio)
        RRPop = EigenVect * R_Allele_Ratio
        ...
        for y in range(Years):
            ...
            for d in range(Days):
                ...
                if d == BreedingTime:
                    ...
                    B_SSPop = copy.deepcopy(SSPop)#SSPop**2 + 2 * 0.5 * SSPop*SRPop + 0.25*SRPop**2
                    
                    B_SRPop = copy.deepcopy(SRPop)#(2 * 0.5 * SSPop*SRPop + 2 * SSPop * RRPop + 
                               #0.5 * SRPop**2 + 2 * 0.5 * RRPop*SRPop)

                    B_RRPop = copy.deepcopy(RRPop)#RRPop**2 + 2 * 0.5 * RRPop*SRPop + 0.25*SRPop**2
        ...
        tempSaveFile = SaveFileName + "/OSR_Proportion_%0.3f"%(OSR_Proportion)

        if not os.path.isdir(tempSaveFile):
            os.mkdir(tempSaveFile)

        xlist = np.arange(len(R_Ratio_Yearly))
        plt.figure(0)
        plt.semilogy(xlist,R_Ratio_Yearly)

        plt.xlabel("Year")
        plt.ylabel("R Allele Ratio")

        plt.grid()

        plt.title("SystemSize_%d"%(SystemSize))
        plt.savefig(tempSaveFile + "/SystemSize_%d"%(SystemSize))
        plt.close()
    ...
    #SmoothedInitialSlopeList =  savgol_filter(InitialSlopeList, 5, 3) # window size 5, polynomial order 3
    windowwidth = int(0.25*len(InitialSlopeList))
    if (windowwidth%2 == 0):
            windowwidth +=1
    print("Window width",windowwidth)
    SmoothedInitialSlopeList =  savgol_filter(InitialSlopeList, windowwidth, 3) # window size 51, polynomial order 3
    ...
    for i in range(1,len(InitialSlopeList)-1):
        if (InitialSlopeList[i+1] > InitialSlopeList[i]) and (InitialSlopeList[i-1] > InitialSlopeList[i]):
            ...
            MinOSR_Proportions.append(OSR_Proportion)
            break
...
#plt.title("OSR Proportion: %0.3f"%(OSR_Proportion))
plt.savefig(SaveFileName + "/MinInitialSlopePlot_LogSystemSize.png")
plt.close()
