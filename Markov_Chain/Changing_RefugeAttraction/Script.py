...
OSR_Proportion = 0.25
Refuge_Attracts = np.arange(0.05,1.05,0.05)
...
Pesticide_Application_Period = 2
Pesticide_SS = 1#0.86
Pesticide_RR = 0.003
dR = 48.6
...
##############################################################################
#SaveFileCreation
SaveFileName = ("Saved_Plots/Years_%d_PAP_%d_gSS_%0.3f_OSR_Proportion_%0.3f_Characteristic_%0.3f_GradientRange_%d_BreedingTime_%d"%
                (Years,Pesticide_Application_Period,Pesticide_SS,OSR_Proportion,Characteristic,GradientRange,BreedingTime))
if FLATPESTICIDE:
...
SmoothedRefuge_Attracts = []

for Refuge_Attract in Refuge_Attracts:
    InitialSlopeList = []
    ...
    for i in range(1,len(InitialSlopeList)-1):
        if (InitialSlopeList[i+1] > InitialSlopeList[i]) and (InitialSlopeList[i-1] > InitialSlopeList[i]):
            ...
            MinRefuge_Attracts.append(Refuge_Attract)
            break
    ...
    if len(minpointindex) > 0:
        ...
        SmoothedRefuge_Attracts.append(Refuge_Attract)
    ...
    plt.title("Refuge Attraction: %0.3f"%(Refuge_Attract))
    plt.savefig(SaveFileName + "/InitialSlopePlot_RefugeAttraction_%0.3f.png"%(Refuge_Attract)) 
    plt.close()
