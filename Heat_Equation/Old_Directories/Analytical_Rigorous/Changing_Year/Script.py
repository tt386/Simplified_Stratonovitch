...
parser = ArgumentParser(description='Different PAP Values')
parser.add_argument('-Y','--Year',type=float,required=True,help='Year')
args = parser.parse_args()

Year = args.Year

YearSaveDirName = SaveDirName + "/Year_" + '{:.1E}'.format(Year)

if not os.path.isdir(YearSaveDirName):
    os.mkdir(YearSaveDirName)
    print("Created Directory for Year",Year)
...
###Output files###############################
##############################################
OutputDatafilename = YearSaveDirName + '/datafile.npz'
