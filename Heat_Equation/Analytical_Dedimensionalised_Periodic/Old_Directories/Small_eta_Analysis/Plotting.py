...
for c in range(len(SlopeMatrix)):
    ...
    theoryList = np.log(2*((1-rho)/2 + (rho/2)*(Phi/(Phi+Phiprime)) + b*np.sqrt(np.pi)*(Phi*np.log((Phiprime+Phi)/Phi) - 1/2 - (Phi/(Phi+Phiprime))/2))) - np.log(Phi)
    ...
    for eta in etaList:
        ...
        AnalyticalSlopeList.append(np.log(2*integrate.quad(lambda x: Phi/(Phi + 0.5+0.5*special.erf((rho-x)/b)), rho/2, (1+rho)/2))-np.log(Phi))

    print("Length off AnalyticalSlopeList",len(AnalyticalSlopeList))

    plt.figure()
    plt.semilogy(etaList,slopelist,label="Data")
    #plt.semilogy(etaList,minmatrix[c],label="Min Point")
    plt.semilogy(etaList,AnalyticalSlopeList,label='Error Function')
    plt.semilogy(etaList,theoryList,label="Linearised Error Function")
