...
for c in range(len(SlopeMatrix)):
    ...
    plt.plot(np.log10(1/np.sqrt(etalist)),np.log10(smoothed),label='Smoother')

    #plt.legend(loc='upper left')

    
    plt.xlabel(r"$\log_{10} \frac{L}{\sqrt{DY}}$",fontsize=30)
    plt.ylabel(r"$\log_{10} \frac{\partial^{2}}{\partial L^{2}} \left(L \Delta R\right)$",fontsize=30)
...
#plt.loglog(PAPList,1/np.sqrt(LeftMostMinima),label='Leftmost')
plt.scatter(np.log10(PhiList[:-1]),np.log10(SecondOrderMaxList[:-1]),color='#4B0092',zorder=5,s=80)#i,label='Rightmost')
...
ax.set_xticks([-6,-3,0])
ax.set_xticklabels([r'$-6$',r'$-3$',r'$0$'])
...
plt.savefig(str(args.directory) + "/LowerKinkWithPhi.eps",bbox_inches="tight")
