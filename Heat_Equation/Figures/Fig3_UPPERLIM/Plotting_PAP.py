...
for c in range(len(SlopeMatrix)):
    ...
    plt.plot(np.log10(1/np.sqrt(etalist)),np.log10(slopelist),
        label="Min L: %0.3f"%((1/np.sqrt(etalist))[slopelist.argmin()]),linewidth=3,color='k')
    ...
    plt.savefig(str(args.directory) + 
        "/Minima_Curve_PAP_" + '{:.1E}'.format(PAP) + ".eps")
   ...
   if PAP <1:
    ...
    plt.plot(np.log10(1/np.sqrt(etalist)),np.log10(smoothed),label='Smoother',linewidth=3,color='k')


    #plt.xlabel(r"$\log_{10} \frac{L}{\sqrt{DY}}$",fontsize=30)
    #plt.ylabel(r"$\log_{10} DY \frac{\partial^{2}}{\partial L^{2}} \left( \frac{\sqrt{DY}}{L} \Delta R\right)$",fontsize=30)
    """
    plt.grid()
    ...
    plt.savefig(str(args.directory) +
        "/SecondOrder_PAP_" + '{:.1E}'.format(PAP) + ".eps")
    plt.close()
...
#plt.loglog(PAPList,1/np.sqrt(LeftMostMinima),label='Leftmost')
plt.scatter(np.log10(1-PAPList[:-1]),np.log10(SecondOrderMaxList[:-1]),color='#4B0092',zorder=5,s=80)#,label='Rightmost')
plt.plot(np.log10(Theoryx),np.log10(Theoryy),'--',color='#1AFF1A',linewidth=5,label='Theory',zorder=0)
...
plt.ylabel(r"$\log_{10}\left(\frac{L_{upper}}{\sqrt{DY}}\right)$",fontsize=30)
...
"""
ax.set_xticks([-0.8,-0.4,0.0])
ax.set_xticklabels([r'$10^{-0.8}$',r'$10^{-0.4}$',r'$10^{0}$'])
"""

ax.set_xticks([-0.8,-0.4,0.0])
ax.set_xticklabels([r'$-0.8$',r'$-0.4$',r'$0$'])

ax.set_yticks([0.6,0.8,1.0,1.2])
ax.set_yticklabels(
    [r'$10^{0.6}$',r'$10^{0.8}$',r'$10^{1.0}$',r'$10^{1.2}$'])
...
plt.figure(2).set_size_inches(6.4, 4.8, forward=True)
plt.figure(2).axes[0].set_position([0.1, 0.1, 0.85, 0.85])

plt.tight_layout()
plt.savefig(str(args.directory) + "/LowerKinkWithPAP.eps",bbox_inches="tight")
