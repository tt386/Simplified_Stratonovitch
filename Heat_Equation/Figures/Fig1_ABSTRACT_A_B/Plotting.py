...
for p in range(len(etalist)):
    ...
    def Setup(plt,ax,xlist,S,R):
        ...
        plt.plot(xlist,np.log10(R),linewidth=10)
    ...
    plt.savefig(str(args.directory) + "1.eps",bbox_inches='tight')
    plt.close()
    ...
    plt.savefig(str(args.directory) + "2.eps",bbox_inches='tight')
    plt.close()
    ...
    plt.savefig(str(args.directory) + "3.eps",bbox_inches='tight')
    plt.close()
    ...
    plt.savefig(str(args.directory) + "4.eps",bbox_inches='tight')
    plt.close()


    #Difference in R
    fig,ax = plt.subplots()
    plt.plot(xlist,np.log10(rlist/(rlist+Yearylist)-rlist),color='orange',linewidth=3)
    plt.fill_between(xlist,np.log10(rlist/(rlist+Yearylist)-rlist),-10,facecolor='orange',alpha=0.5)
    plt.xlim(xlow,xupp)
    plt.ylim(np.log10(ylow),np.log10(yupp))
    AddRects(ax)
    plt.xticks(xticks)
    ...
    ax.axes.xaxis.set_visible(False)
    plt.text(-4*L/10,np.log10(10**-8),r"$\Delta R \left( %d \right)$"%(L),fontsize=30)
    plt.tight_layout() 
    plt.savefig(str(args.directory) + "RDiff.png")
    plt.close()
