...
for c in range(len(SlopeMatrix)):
    ...
    plt.yticks(fontname = "Arial")#rotation=45)

    #plt.title(r"$\omega =%0.1f$"%(Refuge_Proportion),fontsize=30)
    ...
    plt.savefig(NoTheoryDir +
        "/LengthMinima_Curve_rho_" + '{:.1E}'.format(Refuge_Proportion) + ".eps")
    plt.close()
    ...
    try:
        ax.set_xticks([np.log10(SecondOrderDerivList_rho)[-1],np.log10(minSystemSize),np.log10(SecondOrderDerivList_rho_min)[-1]])
        ax.set_xticklabels(
            ['$K_L$',r'$K^*$',r'$K_U$'])
    except:
        print("Upper or Lower bound error")
...
plt.plot(np.log10(x),np.log10(1/np.sqrt(theorykinks)),"--",color='#1AFF1A',label='Theory',linewidth=5,zorder=0)
plt.scatter(np.log10(x),np.log10(1/np.sqrt(y)),color = "#4B0092",s=80,zorder=1)

#plt.legend(loc='lower left',fontsize='20')
#plt.grid()
#plt.xlabel(r"$\log_{10}\left(1-\omega\right)$",fontsize=30)
#plt.ylabel(r"$\log_{10}\frac{l_{upper}}{\sqrt{DY}}$",fontsize=30)
...
ax.set_yticklabels(
    [r'$10^{1.0}$',r'$10^{1.2}$',r'$10^{1.4}$',r'$10^{1.6}$'])
...
plt.xlim(np.log10(0.3),np.log10(1))
...
plt.savefig(NoTheoryDir + "/LengthUpperKink_ChangingRho.eps")
plt.close()
############################################################################
############################################################################
############################################################################
...
#plt.legend(loc='lower left',fontsize='20')
#plt.grid()
#plt.xlabel(r"$\log_{10}\omega$",fontsize=30)
#plt.ylabel(r"$\log_{10}\frac{l_{lower}}{\sqrt{DY}}$",fontsize=30)
...
ax.set_yticks([-0.5,-0.3,-0.1,0.1,0.3])
ax.set_yticklabels(
    [r'$10^{-0.5}$',r'$10^{-0.3}$',r'$10^{-0.1}$',r'$10^{0.1}$',r'$10^{0.3}$'])
...
plt.savefig(NoTheoryDir + "/LengthLowerKink_ChangingRho.eps")
plt.close()
############################################################################
############################################################################
############################################################################
